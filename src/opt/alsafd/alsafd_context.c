...
static void *alsafd_iothd(void *arg) {
  ...
  while (1) {
    ...
    #if MASTER_LEVEL_CHECK_ENABLE
      master_level_check(alsafd->buf,alsafd->bufa,alsafd->rate,alsafd->chanc);
    #endif
...
static int alsafd_select_device_path(
  ...
  if (!(alsafd->device=alsafd_find_device("/dev/snd",ratelo,ratehi,chanclo,chanchi))) {
    //fprintf(stderr,"alsafd_find_device failed (%d..%d,%d..%d)\n",ratelo,ratehi,chanclo,chanchi);
...
static int alsafd_open_device(struct alsafd *alsafd) {
  ...
  fprintf(stderr,"alsafd: Using device '%s'\n",alsafd->device);
